...
pub fn infer_cause_type(description: &str) -> &'static str {
    ...
    if d.contains("spec_ambiguity") || d.contains("spec ambig") || d.contains("ambiguous") || d.contains("acceptance criteria") {
        return "spec_ambiguity";
    }
    if d.contains("twin_gap") || d.contains("twin gap") || d.contains("twin fidelit") {
        return "low_twin_fidelity";
    }
    if d.contains("test_blind") || d.contains("blind spot") || d.contains("happy path") || d.contains("false-negative") {
        return "test_blind_spot";
    }
    if d.contains("no_prior_memory") || d.contains("no prior memory") || d.contains("prior memory") || d.contains("pattern-cold") {
        return "context_gap";
    }
    if d.contains("broad_scope") || d.contains("broad scope") || d.contains("scope reduction") || d.contains("narrow the spec") {
...
pub fn build_scene(run_reports: Vec<(RunRecord, Option<CausalReport>)>) -> TesseractScene {
    ...
    for (run, report) in run_reports {
        ...
        // ── Quality dimensions ────────────────────────────────────────────
        let spec_quality = s.map(|sc| sc.spec_clarity_score).unwrap_or_else(|| {
            match status { "accepted" => 0.72, "in_progress" => 0.55, _ => 0.38 }
        });

        let implementation_quality = s.map(|sc| sc.twin_fidelity_score).unwrap_or_else(|| {
            match status { "accepted" => 0.78, _ => 0.34 }
        });

        let validation_outcome = s.map(|sc| clamp01(
            sc.test_coverage_score * 0.50
            + if sc.scenario_passed  { 0.30 } else { 0.0 }
            + if sc.validation_passed { 0.20 } else { 0.0 },
        )).unwrap_or_else(|| match status { "accepted" => 0.82, _ => 0.18 });
        ...
        let primary_cause_type = primary_cause_text.as_deref()
        ...
        let interventions: Vec<InterventionPlan> = report.as_ref()
            .map(|r| r.recommended_interventions.clone())
            .unwrap_or_default();

        let contributing_causes: Vec<String> = report.as_ref()
        ...
        if let Some(ref cause_text) = primary_cause_text {
            let entry = cause_map.entry(primary_cause_type.clone())
        ...
        for contrib in &contributing_causes {
            ...
            if contrib_type == primary_cause_type { continue; }

            let sec_conf = confidence * 0.55;
            let entry = cause_map.entry(contrib_type.clone())
                .or_insert_with(|| CauseTesseractNode {
